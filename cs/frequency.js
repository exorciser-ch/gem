...
const app = ({ query, store, info }) => {
  ...
  let value = m.stream.merge([store, query]).map(([store, query]) =>  store?.text ?? query?.text ?? || '' )

  let stripwhitespace = m.stream(false)
  let lowercase = m.stream(false)

  let value2 = m.stream.merge([value, stripwhitespace, lowercase])
    .map(([v, sw, l]) => {
      if(sw) {
        v = v.replace(/\s/g, '')
      }
  ...
  let count2 = count.map(c => Object.keys(c)
    .length);
  ...
  return {
    view: () => m(box, {
      ...
      !!(value() || '')
      .length && m('div' + b ` overflow-x: auto`,
        m('svg' + b `ff sans-serif;`, { width: W = (count2() + 1) * D + 1, height: H },
          Array(max() + 1)
          .fill(1)
          .map((_, i) => i)
          .filter(v => v % (Math.ceil(max() / 6)) == 0)
          .map(v => [
          ...
          .map((k, i, _) => [
            ...
            m('text[text-anchor=middle]' + b `fs 50%;`, { x: 1.5 * D + i * D, y: H - D - _ * (H - 2 * D) - D / 4, fill: 'blue' },
              count()[k] / length() < 0.001 ? '<1‰' : (100 * count()[k] / length())
              .toFixed(1) + '%'

            ),

            m('text[text-anchor=middle]', { x: 1.5 * D + i * D, y: H - 0.45 * D }, k),
            m('text[text-anchor=middle]' + b `ff monospace; c purple; fs 50% `, { x: 1.5 * D + i * D, y: H - 0.1 * D }, 'U+' + k.codePointAt(0)
              .toString(16)),
          ])
        )
      ),
